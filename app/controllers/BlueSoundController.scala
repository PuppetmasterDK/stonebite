...
class BlueSoundController @Inject()(cc: ControllerComponents, player: services.Player, configuration: Configuration)(implicit ec: ExecutionContext, fu: play.api.libs.concurrent.Futures) extends AbstractController(cc) with JsonProtocol {
  ...
  private def handleExceptions: PartialFunction[Throwable, Result] = {
    ...
    case e: Exception =>
      ...
      InternalServerError(Json.toJson(Error(None, "PlayerService encountered an exception. Try again later.")))
  }

  def startToPlay(room: String): Action[AnyContent] = Action.async {
    player.play(room).withTimeout(defaultTimeout).map(handleResponse).recover(handleExceptions)
  }

  def pause(room: String): Action[AnyContent] = Action.async {
    player.pause(room).withTimeout(defaultTimeout).map(handleResponse).recover(handleExceptions)
  }

  def volumeUp(room: String): Action[AnyContent] = Action.async {
    player.volumeUp(room).withTimeout(defaultTimeout).map(handleResponse).recover(handleExceptions)
  }

  def volumeDown(room: String): Action[AnyContent] = Action.async {
    player.volumeDown(room).withTimeout(defaultTimeout).map(handleResponse).recover(handleExceptions)
  }
